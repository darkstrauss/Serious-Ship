 ...
 */
 ...
 //lifing
class EventOne extends FlxGroup
{
	...
	{
		//create bounding box for the body. Need one for the torso & head, one for the arm, one for both legs.
		super();
		
		currentState = _currentState;
		
		var headTorsoSizeX:Int = Math.floor(_headTorsoSize.x * (_scaleFactor * 2.1));
		var headTorsoSizeY:Int = Math.floor(_headTorsoSize.y * (_scaleFactor * 1.3));
		
		var armSizeX:Int = Math.floor(_armSize.x * _scaleFactor);
		var armSizeY:Int = Math.floor(_armSize.y * (_scaleFactor * 1.1));
		
		var legSizeX:Int = Math.floor(_legSize.x * (_scaleFactor * 1.2));
		var legSizeY:Int = Math.floor(_legSize.y * (_scaleFactor * 0.9));
		
		boundingCircleOne = new FlxNapeSprite(189, 420);
		...
		boundingCircleTwo = new FlxNapeSprite(287, 521);
	...
	public function trackPosition(_positionForHeadBox:Point, _positionForArmBox:Point, _positionForLegBox:Point)
	{
		...
		boundingBoxes[0].position.x = _positionForHeadBox.x - 60; boundingBoxes[0].position.y = _positionForHeadBox.y;
		
		boundingBoxes[1].position.x = _positionForLegBox.x; boundingBoxes[1].position.y = _positionForLegBox.y;
